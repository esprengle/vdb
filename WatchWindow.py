# coding: utf-8
#.  omz's FilePicker, with added DictTreeNode, dictdialog, and modified cellforrow to use textfield
import ui
import os
...
class EditableTreeDialogController(TreeDialogController):
	def __init__(self, root_node, allow_multi=False, async_mode=False):
		super(self,__init__)(self, root_node, allow_multi, async_mode)

def dict_dialog(rootdict):
	# This is just a demo of how TreeDialogController is
	# extensible with custom TreeNode subclasses, so there
	# aren't as many options as for the regular file dialog.
	root_node = DictTreeNode('globals',rootdict)
	picker = TreeDialogController(root_node, async_mode=False)
	picker.view.present('sheet')
	picker.view.wait_modal()
	if picker.selected_entries:
		return picker.selected_entries[0]
def main():
